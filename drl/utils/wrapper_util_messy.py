"""
Wrapper util adapted from OpenAI baselines.
"""
...
class EpisodicLifeEnv(Wrapper):
    def __init__(self, env):
        ...
        Done by DeepMind for the DQN and co. as it simplifies value estimation.
        """
        super().__init__(self, env)
        self.lives = 0
...
class MaxAndSkipEnv(Wrapper):
    def __init__(self, env, skip=4):
        ...
        obs_buffer_shape = (2, *env.observation_space.shape)
        self._obs_buffer = np.zeros(obs_buffer_shape, dtype=np.uint8)
        self._skip = skip
...
class WarpFrame(ObservationWrapper):
    def __init__(
        ...
        `dict_space_key` can be specified to indicate which
...
class FrameStack(Wrapper):
    def __init__(self, env, k):
        ...
        stacked_shape = (*shape[:-1], shape[-1] * k)
        self.observation_space = Box(
            low=0, high=255, shape=stacked_shape,
