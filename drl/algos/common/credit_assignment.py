...
def gae_advantages(seg_len, extra_steps, gamma, lam, rewards, vpreds, dones, use_dones=True):
    ...
    for t in reversed(range(0, seg_len+extra_steps)):  # T+(n-1)-1, ..., 0
        ...
        nonterminal_t = (1.-dones[t]) if use_dones else 1.
        delta_t = -V_t + r_t + nonterminal_t * gamma * V_tp1
        A_t = delta_t + nonterminal_t * gamma * lam * A_tp1
        advantages[t] = A_t
    return advantages


def nstep_advantages(seg_len, extra_steps, gamma, rewards, vpreds, dones, use_dones=True):
    ...
    for t in reversed(range(0, seg_len)):  # T-1, ..., 0
        ...
        for s in reversed(range(0, extra_steps+1)):  # ((n-1)+1)-1 = n-1, ..., 0
            ...
            nonterminal_tps = (1.-dones[t+s]) if use_dones else 1.
            R_t = r_tps + nonterminal_tps * gamma * R_t
        V_t = vpreds[t]
